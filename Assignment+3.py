...
def energy(): 
    energy = pd.read_excel('Energy Indicators.xls', header=None, skiprows=18, skip_footer=38,na_values='...')
    energy = energy.loc[:,2:]
...
def gdp():
    ...
    #change column name "Country Name" to "Country"
    GDP.rename(columns={'Country Name': 'Country'}, inplace=True)
    
    #GDP['Country'] = GDP['Country'].apply(lambda x: x.strip())
    GDP['Country'] = GDP['Country'].str.replace(r" \(.*\)","")
    
    #keep only the column ['Country', '2006', '2007', '2008','2009','2010', '2011','2012','2013','2014','2015']
...
def scimen():
    ...
    #scimen['Country'] = scimen['Country'].apply(lambda x: x.strip())
...
def answer_one():
    ...
    tdf = pd.merge(Energy, GDP, how = 'inner', left_on='Country', right_on='Country')
    #tdf = inner merge tdf and ScimEn on 'Country'
    tdf = pd.merge(ScimEn, tdf, how = 'inner', left_on='Country', right_on='Country')
...
def answer_two():
  #call the function
    ...
    tdf_in = pd.merge(Energy, GDP, how = 'inner', left_on='Country', right_on='Country')
    tdf_in = pd.merge(ScimEn, tdf_in, how = 'inner', left_on='Country', right_on='Country')

    tdf_out = pd.merge(Energy, GDP, how = 'outer', left_on='Country', right_on='Country')
    tdf_out = pd.merge(ScimEn, tdf_out, how = 'outer', left_on='Country', right_on='Country')
...
def answer_four():
    top15 = answer_one()
    res = top15.loc['United Kingdom','2015'] - top15.loc['United Kingdom','2006']
    return res
...
def answer_five():
    Top15 = answer_one()
    avgESP = Top15['Energy Supply per Capita'].mean()
    return avgESP
...
def answer_six():
    Top15 = answer_one()
    top_percent = Top15.sort_values(by='% Renewable', ascending=False).iloc[0]
    return (top_percent.name, top_percent['% Renewable'])
...
def answer_seven():
    Top15 = answer_one()
    Top15['Ratio of Self Citations'] = Top15['Self-citations']/Top15['Citations']
    top_ratio = Top15.sort_values(by='Ratio of Self Citations', ascending=False).iloc[0]
    return (top_ratio.name, top_ratio['Ratio of Self Citations'])
...
def answer_eight():
    ...
    pop_est = Top15.sort_values(by='Pop Estimate', ascending=False).iloc[2]
    return (pop_est.name)
...
def answer_nine():
    Top15 = answer_one()
    Top15['Pop Estimate'] = Top15['Energy Supply']/Top15['Energy Supply per Capita']
    Top15['Citable docs per Capita'] = Top15['Citable documents'] / Top15['Pop Estimate']
